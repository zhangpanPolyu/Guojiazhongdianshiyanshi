...
import type { Alert, Metric } from "@workspace/api-client-react";
    ...
    <View
      style={[
        styles.metricCard,
        {
          ...
          borderColor: isWarning ? "#FFB800" + "44" : isGood ? "#00FF6622" : colors.border,
        },
      ]}
    >
      ...
      <Text style={[styles.metricValue, { color: colors.sciCyan }]}>
        {metric.value.toFixed(1)}
        <Text style={[styles.metricUnit, { color: colors.mutedForeground }]}> {metric.unit}</Text>
      </Text>
    </View>
  );
}
  ...
  return (
    ...
    >
      <View style={styles.alertRow}>
        <View style={[styles.alertBadge, { borderColor: alert.acknowledged ? colors.border : sevColor }]}>
          <Text style={[styles.alertBadgeText, { color: alert.acknowledged ? colors.mutedForeground : sevColor }]}>
            {alert.severity.toUpperCase()}
          </Text>
        </View>
        <Text style={[styles.alertTime, { color: colors.mutedForeground }]}>{timeAgo(alert.timestamp)}</Text>
        {alert.acknowledged && (
          <Feather name="check-circle" size={12} color={colors.mutedForeground} />
        )}
      </View>
      <Text style={[styles.alertMsg, { color: alert.acknowledged ? colors.mutedForeground : colors.foreground }]}>
        {alert.messageEn}
      </Text>
    </View>
  );
...
export default function EquipmentDetailScreen() {
  ...
  if (isError || !equipment) {
    return (
      <View style={[styles.container, styles.centered, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.errorText, { color: colors.sciRed }]}>Equipment not found</Text>
  ...
  return (
    ...
    >
      <View style={[styles.heroSection, { borderBottomColor: statusColor + "44", borderBottomWidth: 1 }]}>
        ...
        <Text style={[styles.equipmentName, { color: colors.foreground }]}>{equipment.nameEn}</Text>
        {equipment.name !== equipment.nameEn && (
          <Text style={[styles.equipmentNameAlt, { color: colors.mutedForeground }]}>{equipment.name}</Text>
        )}
      </View>

      <View style={[styles.section, { borderColor: colors.border }]}>
        <Text style={[styles.sectionTitle, { color: colors.sciCyan }]}>DETAILS</Text>
        <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <InfoRow icon="map-pin" label="Location" value={equipment.locationEn ?? equipment.location} />
          ...
          {equipment.serialNumber && (
            <>
              <View style={[styles.divider, { backgroundColor: colors.border }]} />
              <InfoRow icon="hash" label="Serial" value={equipment.serialNumber} />
            </>
          )}
          <View style={[styles.divider, { backgroundColor: colors.border }]} />
          <InfoRow
          ...
          {equipment.predictedFailureDate && (
            <>
              ...
              <InfoRow
                ...
                value={new Date(equipment.predictedFailureDate).toLocaleDateString()}
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.sciCyan }]}>LIVE METRICS</Text>
        {metricsLoading ? (
          ...
          <View style={styles.metricsEmpty}>
            ...
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No metrics available</Text>
          </View>
        ) : (
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.sciCyan }]}>ALERT HISTORY</Text>
        {alertsLoading ? (
          <View style={styles.metricsLoading}>
            <ActivityIndicator color={colors.sciCyan} />
            <Text style={[styles.loadingText, { color: colors.mutedForeground }]}>Loading alerts...</Text>
          </View>
        ) : !equipmentAlerts || equipmentAlerts.length === 0 ? (
          <View style={[styles.metricsEmpty, { backgroundColor: colors.card, borderRadius: 10, borderWidth: 1, borderColor: colors.border }]}>
            <Feather name="bell-off" size={28} color={colors.mutedForeground} />
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No alerts for this equipment</Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  ...
  heroSection: { paddingHorizontal: 16, paddingVertical: 20, paddingBottom: 20 },
  heroTop: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 },
  statusIndicator: { width: 8, height: 8, borderRadius: 4 },
  statusLabel: { fontSize: 11, fontFamily: "Inter_700Bold", letterSpacing: 1.5 },
  equipmentName: { fontSize: 24, fontFamily: "Inter_700Bold", lineHeight: 30 },
  equipmentNameAlt: { fontSize: 14, fontFamily: "Inter_400Regular", marginTop: 4 },
  ...
  alertBadge: { borderWidth: 1, borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1 },
  alertBadgeText: { fontSize: 10, fontFamily: "Inter_700Bold", letterSpacing: 0.5 },
  alertTime: { fontSize: 11, fontFamily: "Inter_400Regular", flex: 1 },
